...
W_conv1 = weight_variable([1, 7, 1, 16])
...
h_pool3 = max_pool_1x4(h_conv3)

W_fc1 = weight_variable([1*16*64,128])
...
h_pool2_flat = tf.reshape(h_pool3, [-1, 16 * 64])
...
center_loss =  (Sw / Sb) #(1-tf.exp(-Sw)) + 20*tf.exp(-Sb) # Sw / Sb
...
loss = cross_entropy #+ 1000.0 * center_loss #+ 0.0001 * dist
# 使用Adam优化算法来调整参数
