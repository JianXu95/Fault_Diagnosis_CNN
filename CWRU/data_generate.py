...
class CWRU_Data:
    def __init__(self, length, m, snr=None, FE=0):
        # root directory of all data
        rdir = os.path.join('..\Datasets\CWRU')
        all_lines = open('..\Datasets\metadata7.txt').readlines()
        lines = []
    ...
    def _slice_data(self, rdir, infos, FE):
        ...
        for idx, info in enumerate(infos):
            ...
            for sample in range(samples):
                ...
                self.y = np.vstack((self.y, idx%7))

        self.X_train, self.X_test, self.y_train, self.y_test\
            = train_test_split(self.X, self.y, test_size=3500)
...
if __name__ == '__main__':
    ...
    data = CWRU_Data(length=1024, m=150, snr=0, FE=0)
